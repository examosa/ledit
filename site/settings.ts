                  ...
                  ${map(
                     ...
                     (bookmark, index) => html`
                        <div class="flex items-center gap-4 px-4 border-b border-border/50 h-[3em]" data-index="${index.toString()}">
                           ...
                           <i
                              class="icon ml-auto block min-w-[1.5em] min-h-[1.5em] ${bookmark.isDefault ? "fill-primary" : "fill-primary/30"}"
                              @click=${() => this.defaultClicked(bookmark)}
                              >${checkmarkIcon}</i
                           >
...
export class BookmarkEditor extends LitElement {
   ...
   render() {
      ...
      return html`<ledit-overlay
         id="overlay"
         headerTitle="${isNew ? `New ${sourcePrefix} bookmark` : `Edit ${sourcePrefix} bookmark`}"
